...
<template v-slot:activator="{ on }">
  <div>
    ...
    <v-toolbar color="primary">
      <v-spacer></v-spacer>
      <v-spacer></v-spacer>
      <v-toolbar-title dark> {{ appTitle }}</v-toolbar-title>
      <v-spacer></v-spacer>
      <v-toolbar-items>
        <v-btn
          text
        ...
        <!-- <v-menu v-if="admin" offset-y>
          <template v-slot:activator="{ on }">
            <v-btn
              ...
              text
              class="btnAdmin hidden-sm-and-down"
              color="yellow darken-3--text"
            >
              <v-icon> mdi-poll </v-icon>
              &nbsp;{{ $t('metricItems.METRICS') }}
            </v-btn>
          </template>
          <v-list>
            <v-list-item
    ...
    <!-- <v-navigation-drawer v-model="sidebar" absolute disable-resize-watcher>
      <v-list>
        ...
        <v-list-group v-if="admin" prepend-icon="mdi-stack-overflow" no-action>
          <v-list-item slot="activator" class="pl-0">
            <v-list-item-content>{{
              $t('controlItems.CONTROL')
            }}</v-list-item-content>
        ...
        <v-list-group v-if="admin" prepend-icon="mdi-poll" no-action>
          <v-list-item slot="activator" class="pl-0">
            <v-list-item-content>{{
              $t('metricItems.METRICS')
            }}</v-list-item-content>
