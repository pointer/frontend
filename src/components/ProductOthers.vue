<template>
  <v-dialog v-model="othersDlg" max-width="800px">
    <v-card>
      <v-card-title>
            ...
            <v-card-text>
              <v-container>
                <v-row>
                  ...
                  <v-col cols="12" sm="6" md="4">
                    <v-text-field
                      v-model="editedItem.product_weigth"
                      :label="$t('product.headers.PRODUCT_WEIGTH')"
                    ></v-text-field>
                  </v-col>
                  <v-row>
                ...
                <v-row>
                  <v-col>
                    <v-text-field
                      v-model="editedItem.other_product_details"
...
export default {
  ...
  created() {
    const that = this
    bus.$on('othersDlg', (value) => {
      that.othersDlg = value
    })
  }
}
</script>
