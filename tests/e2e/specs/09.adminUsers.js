...
describe('Admin Users', () => {
  ...
  it('Checks input types for create/edit new user', () => {
    ...
    cy.get('input[name=role]').invoke('attr', 'type').should('contain', 'text')
  ...
  it('Create new user', () => {
    ...
    cy.get('div.v-list__tile__title').contains('Administrator').click()
    cy.get('div.inputRole > div > div > div > div > div > i.mdi-close').click()
    cy.get(
      'div.inputRole > div > div > div > div > div > i.mdi-menu-down'
    ).click()
    cy.get('div.v-list__tile__title').contains('User').click()
    // city
    cy.get(
      'div.inputCity > div > div > div > div > div > i.mdi-menu-down'
    ).click()
    cy.get('div.v-list__tile__title').contains('Cali').click()
    cy.get('div.inputCity > div > div > div > div > div > i.mdi-close').click()
    cy.get(
      'div.inputCity > div > div > div > div > div > i.mdi-menu-down'
    ).click()
    cy.get('div.v-list__tile__title').contains('Bucaramanga').click()
    // country
  ...
  it('Edit user', () => {
    ...
    // Search user
    cy.get('input[aria-label=Search]').clear().type('A New User')
    cy.wait(1000)
    // Click and edit first element
    cy.get('td > div > span.v-tooltip.v-tooltip--top').eq(0).click()
    // name
  ...
  it('Search user', () => {
    ...
    // Search edited user
    cy.get('input[aria-label=Search]').clear().type('A New User Edited')
  ...
  it('Delete user', () => {
    ...
    cy.get('td > div > span.v-tooltip.v-tooltip--top').eq(1).click()
    cy.get('div.v-dialog.v-dialog--active').should('be.visible')
    cy.get('button > div.v-btn__content').contains('Delete').click()
